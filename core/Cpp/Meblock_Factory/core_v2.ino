...
// Lệnh BOOT_APP0 / APP0 / FACTORY → nhảy sang OTA_0 (app0)
void cmdBootApp0() {
  Serial.println("[MEBLOCK] Switching boot partition to app0 (OTA_0 / FACTORY)...");
  ...
  if (!app0) {
    Serial.println("[MEBLOCK] ERROR: app0 (OTA_0) partition not found!");
    return;
  }
  ...
  Serial.printf(
      ...
      app0->label,
      (unsigned long)app0->address,
      (unsigned long)app0->size
  );

  esp_err_t err = esp_ota_set_boot_partition(app0);
  if (err == ESP_OK) {
    Serial.println("[MEBLOCK] Restarting to app0...");
    delay(200);
    ESP.restart();
  } else {
...
// Lệnh BOOT_APP1 / APP1 → nhảy sang OTA_1 (app1)
void cmdBootApp1() {
...
bool handleMeblockCommand(const String &cmdIn) {
  ...
  // Boot sang app0 / factory
  if (cmd.equalsIgnoreCase("BOOT_APP0") || cmd.equalsIgnoreCase("APP0") ||
      cmd.equalsIgnoreCase("RESET_FACTORY") || cmd.equalsIgnoreCase("FACTORY")) {
    cmdBootApp0();
    return true;
  }
...
// ===== BOOT button detector: double BOOT -> APP0 =====
static const int BOOT_BUTTON_PIN = 0;
static const uint32_t DOUBLE_BOOT_WINDOW_MS = 3000;
static const uint32_t DEBOUNCE_MS = 60;

static bool checkDoubleBootToApp0AtStartup() {
  pinMode(BOOT_BUTTON_PIN, INPUT_PULLUP);

  Serial.println("[BOOT] Press BOOT twice to boot app0...");
...
static bool isMeblockTextCommand(const String &cmd) {
  ...
  if (cmd.equalsIgnoreCase("BOOT_APP0")) return true;
  if (cmd.equalsIgnoreCase("APP0")) return true;
  if (cmd.equalsIgnoreCase("RESET_FACTORY")) return true;
  if (cmd.equalsIgnoreCase("FACTORY")) return true;
...
void setup() {
  ...
  if (checkDoubleBootToApp0AtStartup()) {
    Serial.println("[BOOT] Double BOOT detected -> boot app0");
    cmdBootApp0();
  }
