...
def _sanitize_frame_token(s):
    ...
    if s is None:
        s = ""
    s = str(s)
    # Never allow ACK inside payload (avoid breaking frame parsing)
    s = s.replace(_ACK, "")
    # Avoid "/" because UI splits by "/"
    s = s.replace("/", "_")
    # Keep frame one-line friendly (optional)
...
def _send_frame(_type, code, text, out=None):
    ...
    if callable(out):
        try:
            ...
            # Some streams expect str
            out(frame_str)
            return frame_bytes
        except Exception:
            return frame_bytes

    # object.write / object.send
    for m in ("write", "send"):
...
def send_inf(code_or_text, text=None, out=None):
    ...
    Send INFO frame.
    Usage:
      send_inf("Hello")                    -> inf/I_TEXT/Hello/
      send_inf("I_STATUS", "Init...")      -> inf/I_STATUS/Init.../
    """
    if text is None:
        code = "I_TEXT"
        text = code_or_text
...
def send_war(code_or_text, text=None, out=None):
    ...
    Send WARNING frame.
    Usage:
      send_war("Pin yếu: 3.45V")           -> war/W_LOW_BAT/Pin yếu: 3.45V/
      send_war("W_LOW_BAT", "3.45V")       -> war/W_LOW_BAT/3.45V/
    """
    if text is None:
        code = "W_LOW_BAT"
        text = code_or_text
...
def send_err(code_or_text, text=None, out=None):
    ...
    Send ERROR frame.
    Usage:
      send_err("DHT20 không phản hồi")      -> err/E_DEV/DHT20 không phản hồi/
      send_err("E_WIFI", "Sai mật khẩu")   -> err/E_WIFI/Sai mật khẩu/
    """
    if text is None:
        code = "E_DEV"
        text = code_or_text
