...
export class BleMicroPythonNUS {
  ...
  _onTxNotify(event) {
    ...
    let text = this.dec.decode(value);

    // Giảm nhiễu log khi vào raw REPL
    // (MicroPython thường in: "raw REPL; CTRL-B to exit")
    text = text.replace(/raw REPL; CTRL-B to exit\r?\n?/g, '');
  ...
  // ===== RAW REPL helpers (giảm lặp Ctrl-A / "CTRL-B to exit") =====
  async rawBegin() {
    this._requireConnected();
    await this.ctrlC();
    await this.ctrlA();
  ...
  // Execute code assuming we are already in raw REPL
  async rawExecInRaw(code) {
    this._requireConnected();
  ...
  async rawExec(code) {
    ...
    // Vào raw REPL 1 lần cho mỗi lần exec
    await this.rawBegin();
    await this.rawExecInRaw(code);
  }
  ...
  async flashFile(file, { dst = 'main.py', onProgress } = {}) {
    ...
    // Vào raw REPL 1 lần, tránh lặp lại thông báo "CTRL-B to exit" mỗi chunk
    await this.rawBegin();
...
def __fw(path, data_b64, mode):
    ...
    f.close()
`;
    await this.rawExecInRaw(helper);
    ...
    // Không gửi CTRL-B khi flash (theo yêu cầu) — giữ nguyên trạng thái REPL.
  }
