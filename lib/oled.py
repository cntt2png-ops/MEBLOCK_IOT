...
#   hoặc:
#   oled = Oled(board="s3", driver="SH1106")
...
def _pick_pins(board, sda, scl):
    if sda is not None and scl is not None:
        return sda, scl
    if board:
        b = board.lower()
        if b in _BOARD_PRESETS:
            return _BOARD_PRESETS[b]["sda"], _BOARD_PRESETS[b]["scl"]
        raise ValueError("Board khong ho tro preset: {}".format(board))
    # default an toàn (ESP32 WROOM)
    return 21, 22
...
def _make_i2c(i2c, backend, i2c_id, sda, scl, freq):
    ...
    if be == "auto":
        ...
        if _SoftI2C is None:
            raise RuntimeError("SoftI2C khong kha dung tren firmware nay")
        use_soft = True
    elif be == "hard":
        use_soft = False
    else:
        raise ValueError('backend phai la "soft" | "hard" | "auto"')
    if use_soft:
...
def _auto_addr(i2c, addr):
    ...
    devs = i2c.scan()
    if not devs:
        raise RuntimeError("Khong thay thiet bi I2C. Kiem tra day/nguon.")
    return 0x3C if 0x3C in devs else (0x3D if 0x3D in devs else devs[0])

# Factory tương thích với oledx.create
def create(*,
           board=None,
           i2c=None, i2c_id=0, i2c_sda=None, i2c_scl=None, i2c_freq=400_000,
           width=128, height=64,
           ctrl="SSD1306", sh1106_col_offset=2,
           addr=None, backend="soft",
           debug=False):
    sda, scl = _pick_pins(board, i2c_sda, i2c_scl)
    i2c_obj = _make_i2c(i2c, backend, i2c_id, sda, scl, i2c_freq)
    ...
    if ctrl_up == "SSD1306":
        ...
        raise ValueError('ctrl phai la "SSD1306" hoac "SH1106"')
...
class Oled:
    def __init__(self, width=128, height=64, *,
                 ...
                 debug=False):
        self.width, self.height = width, height
        self.dev = create(board=board,
                          i2c=None, i2c_id=i2c_id,
                          i2c_sda=sda, i2c_scl=scl, i2c_freq=freq,
    ...
    # ủy quyền các API framebuffer còn lại (pixel, line, rect, fill_rect, ...)
    def __getattr__(self, name):
        return getattr(self.dev, name)

    def info(self):
        try: iid = self.dev.i2c.id()
        except: iid = None
        return {"width": self.width, "height": self.height, "addr": hex(getattr(self.dev, "addr", 0)),
                "i2c_id": iid}
